...
// NewSingleNodeValkeyStorage creates a new Valkey storage instance for single-node deployment
func NewSingleNodeValkeyStorage(valkeyURI string) (*ValkeyStorage, error) {
	log := log.WithField("prefix", "NewSingleNodeValkeyStorage")
	...
	log.Info("Successfully connected to Valkey (single-node mode)")
...
// NewClusteredValkeyStorage creates a new Valkey storage instance for cluster deployment
// Discovers cluster topology using CLUSTER SLOTS command
func NewClusteredValkeyStorage(valkeyURI string) (*ValkeyStorage, error) {
	log := log.WithField("prefix", "NewClusteredValkeyStorage")
	...
	if err != nil {
		tempClient.Close()
		return nil, fmt.Errorf("failed to discover cluster topology via CLUSTER SLOTS: %w", err)
	}
	...
	if len(clusterSlots) == 0 {
		return nil, fmt.Errorf("CLUSTER SLOTS returned empty result - not a valid cluster")
	}
	...
	log.Infof("Discovered %d cluster nodes: %v", len(addrs), addrs)
	...
	log.Infof("Successfully connected to Valkey cluster with %d nodes", len(addrs))
	return &ValkeyStorage{
		client:      client,
		subscribers: make(map[string][]chan<- models.SseMessage),
	}, nil
