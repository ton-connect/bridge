...
# Script to run tests on 6-node Valkey cluster
# Usage: ./run-cluster-6-nodes.sh [write-heavy|read-heavy|mixed]

set -e

SCENARIO=${1:-mixed}
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.cluster-6-nodes.yml"
...
mkdir -p "$SCRIPT_DIR/results/cluster-6-nodes"
...
docker-compose -f "$COMPOSE_FILE" up -d valkey-shard1 valkey-shard2 valkey-shard3 valkey-shard4 valkey-shard5 valkey-shard6 valkey-cluster-init bridge data-monitor
...
# Save test metadata
RESULT_FILE="$SCRIPT_DIR/results/cluster-6-nodes/test-metadata-$SCENARIO.txt"
...
echo "Cluster: 6-node (0.90 CPU total)" >> "$RESULT_FILE"

# Rename summary file to include scenario
if [ -f "$SCRIPT_DIR/results/cluster-6-nodes/summary.json" ]; then
  mv "$SCRIPT_DIR/results/cluster-6-nodes/summary.json" \
     "$SCRIPT_DIR/results/cluster-6-nodes/summary-$SCENARIO.json"
fi

echo ""
echo "========================================"
echo "Test completed: $SCENARIO"
echo "Results saved to: results/cluster-6-nodes/"
echo "========================================"
echo ""

# Show quick summary
if [ -f "$SCRIPT_DIR/results/cluster-6-nodes/summary-$SCENARIO.json" ]; then
  echo "Quick Summary:"
  cat "$SCRIPT_DIR/results/cluster-6-nodes/summary-$SCENARIO.json" | \
